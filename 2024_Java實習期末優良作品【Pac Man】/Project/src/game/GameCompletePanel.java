...
public class GameCompletePanel extends JPanel {
    ...
    public Clip levelWinSoundClip; // Level win sound effect
    public boolean isSoundPlayed = false; // Flag variable to prevent the sound from playing repeatedly

    // Constructor that receives the main frame and final score
    public GameCompletePanel(JFrame frame, int finalScore) {
        this.frame = frame;
        this.finalScore = finalScore;
        setLayout(new BorderLayout()); // Set layout manager
        loadBackgroundImage("/win.jpg"); // Load background image
        ...
        playLevelWinSound(); // Play level win sound effect
    }
    ...
    // Method to play level win sound effect
    public void playLevelWinSound() {
        if (!isSoundPlayed) { // Check if the sound has already been played
            try {
                URL url = getClass().getResource("/level-win.wav"); // Load sound file
                if (url == null) {
                    throw new IOException("Audio file not found");
                }

                AudioInputStream audioIn = AudioSystem.getAudioInputStream(url);
                levelWinSoundClip = AudioSystem.getClip();
                levelWinSoundClip.open(audioIn);
                levelWinSoundClip.start();
